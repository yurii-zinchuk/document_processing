...
class ProcessingNotificationManager @Inject constructor(
    ...
    fun getNotification(title: String, progress: Boolean = true): Notification =
        NotificationCompat
    ...
    fun sendSuccessNotification(title: String, workName: String) {
    ...
    fun sendFailureNotification(title: String, workName: String) {
        (context.getSystemService(NOTIFICATION_SERVICE) as NotificationManager).notify(
            SUCCESS_NOTIFICATION_ID + UUID.fromString(workName).hashCode(),
            getNotification(title, false),
        )
